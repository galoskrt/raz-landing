...
(function () {
  ...
  function onScroll() {
    ...
    [25, 50, 75, 95].forEach(function (mark) {
      if (deepest >= mark && !sent[mark]) { sent[mark] = 1; report(mark); }
    });
  }
  // the Worker fills this in once the personal link is live
  function report(depth) {
    if (!window.RAZ_TOKEN) return;
    navigator.sendBeacon('/api/read', JSON.stringify({ t: window.RAZ_TOKEN, depth: depth }));
  }
